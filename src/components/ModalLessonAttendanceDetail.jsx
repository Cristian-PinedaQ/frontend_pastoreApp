...
const ModalLessonAttendanceDetail = ({
  ...
  const loadContent = useCallback(async () => {
    ...
    if (!eId || !lId) {
      console.warn("⚠️ [ModalLessonAttendanceDetail] IDs faltantes:", {
        eId,
        lId,
      });
      return;
    ...
    try {
      ...
      if (!activeLesson) {
        try {
          activeLesson = await apiService.getLessonById(lId);
        } catch (err) {
      ...
      if (!activeEnrollment) {
        try {
          activeEnrollment = await apiService.getEnrollmentById(eId);
        } catch (err) {
          console.error("Error cargando cohorte:", err);
        }
      }
      ...
      const [enrollmentStudents, attData] = await Promise.all([
        apiService.getStudentEnrollmentsByEnrollment(eId),
        apiService.getAttendancesByLesson(lId).catch(() => []),
      ]);

      // Filtrar cancelados (misma lógica que el original)
      const allStudents = Array.isArray(enrollmentStudents)
        ? enrollmentStudents
        : [];
  ...
  const handleQuickAttendance = async (studentEnrollmentId, present = true) => {
    ...
    try {
      ...
      const score =
        participationScores[studentEnrollmentId] || "POCA_PARTICIPACION";
    ...
    >
      ...
      >
        ...
        <div className="p-7 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-br from-indigo-600 to-violet-700 text-white relative overflow-hidden shrink-0">
            ...
            <div className="space-y-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold opacity-80 uppercase tracking-wide">
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {currentEnrollment?.cohortName ||
                    enrollmentProp?.cohortName ||
                    "Cargando..."}
                </span>
              </div>
            </div>
          ...
          <button
            onClick={onClose}
            className="absolute top-5 right-5 flex items-center justify-center bg-black/20 hover:bg-black/40 rounded-2xl transition-all z-20"
          >
          ...
          {loading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-xs text-slate-400 font-semibold mt-1">
                No hay estudiantes activos en esta cohorte.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {students.map((student, idx) => {
                ...
                const name =
                  student.memberName ||
                  student.name ||
                  `Estudiante ${student.memberId || student.id}`;
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                      {att ? (
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wide
                          ...
                          {att.recordedBy && (
                            <span className="text-[10px] text-slate-400 italic font-semibold hidden md:block">
                              por {att.recordedBy}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            onClick={() =>
                              handleQuickAttendance(student.id, true)
                            }
