 ...
 * MemberDetailModal
 * Fixes applied:
 *  1. Split loading state: `loading` (enrollment list) vs `detailLoading` (detail fetch)
 *     — fetching a detail no longer hides the card list behind the spinner.
 *  2. `error` is cleared before every fetch and on every tab switch
 *     — stale error banners no longer persist across interactions.
 *  3. Detail button is disabled (with spinner) while `detailLoading` is true
...
export const MemberDetailModal = ({
  ...
  const [loading, setLoading] = useState(true);         // enrollment list fetch
  const [detailLoading, setDetailLoading] = useState(false); // single-detail fetch
  ...
  const loadEnrollments = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null); // ── FIX 2: clear stale error on list reload ──────────
  ...
  // ── FIX 1 + 2: uses detailLoading, clears error before each attempt ─────────
  const handleViewEnrollmentDetail = async (enrollment) => {
    ...
    try {
      ...
      if (lessons.length === 0 && enrollment.enrollment?.id) {
  ...
  const getStatusBadge = (status) => {
    const configs = {
      ...
      FAILED:    { color: "bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border border-orange-100 dark:border-orange-800/50",   text: "Reprobado",  icon: AlertCircle },
    };
    const config = configs[status] || {
      color: "bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200",
      text: status,
      icon: History
    };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 lg:p-6 animate-fade-in">
      ...
      <div className="relative w-full max-w-5xl bg-white dark:bg-[#1a2332] rounded-[2rem] md:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] border border-slate-200 dark:border-slate-800 animate-slide-up">
        ...
        <div className="relative p-6 md:p-8 lg:p-10 flex flex-col md:flex-row items-center md:items-start gap-5 md:gap-8 border-b border-slate-100 dark:border-slate-800/80 overflow-hidden shrink-0">
          <div className="absolute top-0 right-0 w-48 h-48 md:w-80 md:h-80 bg-indigo-500/5 rounded-bl-[10rem] md:rounded-bl-[15rem] -mr-10 -mt-10 md:-mr-20 md:-mt-20 pointer-events-none" />
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 overflow-x-auto no-scrollbar shrink-0">
          {[
            ...
            <button
              ...
              className={`px-6 md:px-8 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-widest transition-all relative shrink-0 ${activeTab === key ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
            >
              {label}
              {activeTab === key && <div className="absolute bottom-0 left-0 w-full h-[3px] bg-indigo-600 rounded-t-full" />}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 lg:p-10 custom-scrollbar relative">
          ...
          {activeTab === "info" && (
            <div className="space-y-8 md:space-y-10 animate-fade-in">
              ...
              <div className="pt-6 md:pt-8 border-t border-slate-200 dark:border-slate-800/80 flex flex-col md:flex-row flex-wrap gap-3 md:gap-4">
                {canEdit && (
                  <>
                    <button
                      ...
                      className="w-full md:w-auto justify-center px-6 md:px-8 py-3.5 md:py-4 bg-indigo-600 text-white rounded-[1.2rem] md:rounded-2xl font-bold text-[10px] md:text-xs uppercase tracking-wider flex items-center gap-2.5 shadow-lg shadow-indigo-500/20 active:scale-95 transition-all group"
                    >
                    ...
                    <button
                      onClick={async () => {
                        await confirm({
                          ...
                          onConfirm: async () => { onDelete(member.id); }
                        });
                      }}
                ...
                {onViewEnrollment && (
                  <button
                    onClick={() => { onViewEnrollment(member.id, member.name); onClose(); }}
                    className="w-full md:w-auto justify-center px-6 md:px-8 py-3.5 md:py-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-[1.2rem] md:rounded-2xl font-bold text-[10px] md:text-xs uppercase tracking-wider flex items-center gap-2.5 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all md:ml-auto"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  {enrollments.map(enroll => (
                    <EnrollmentCard
                      key={enroll.id}
                      enroll={enroll}
                      // ── FIX 1: pass detailLoading so button can show spinner ──
          ...
          {activeTab === "enrollmentDetail" && enrollmentDetail && (
            <div className="space-y-6 md:space-y-8 animate-fade-in">
              <button
                onClick={() => { handleTabChange("enrollments"); setEnrollmentDetail(null); }}
              ...
              <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-40 h-40 md:w-64 md:h-64 bg-white/10 rounded-bl-[10rem] -mr-10 -mt-10 md:-mr-16 md:-mt-16 pointer-events-none" />
                <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 md:gap-8">
                  ...
                  <div className="w-full sm:w-auto text-center px-6 md:px-8 py-4 md:py-6 bg-white/10 backdrop-blur-md rounded-[1.5rem] md:rounded-[2rem] border border-white/20 shrink-0">
                    ...
                    <p className="text-3xl md:text-4xl font-black tracking-tighter">
                      {(enrollmentDetail.attendancePercentage || 0).toFixed(1)}<span className="text-xl md:text-2xl ml-0.5 opacity-80">%</span>
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* ── FIX 2: error is always fresh (cleared before each fetch) ──────── */}
          {error && (
...
// ── FIX 1: receives detailLoading to disable button and show spinner ───────────
const EnrollmentCard = ({ enroll, onView, onWithdraw, getLevelName, getStatusBadge, detailLoading }) => (
  <div className="group bg-white dark:bg-slate-800/30 rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 dark:border-slate-700/80 p-5 md:p-6 lg:p-8 shadow-sm hover:shadow-xl hover:border-indigo-200 dark:hover:border-indigo-800/50 transition-all flex flex-col gap-4 md:gap-5 relative overflow-hidden">
    <div className="absolute top-0 left-0 w-1 md:w-1.5 h-full bg-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity" />
    ...
    <div className="flex justify-between items-start gap-3 md:gap-4">
      <div className="space-y-1 md:space-y-1.5 min-w-0 flex-1">
        ...
        <p className="text-[9px] md:text-[10px] font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <span className="line-clamp-1">{getLevelName(enroll.enrollment?.level)}</span>
        </p>
      </div>
    ...
    <div className="flex items-center gap-4 md:gap-6 py-3 md:py-4 border-y border-slate-100 dark:border-slate-700/50">
      <div className="flex-1 text-center">
        ...
        <p className="text-lg md:text-xl font-black text-indigo-600 dark:text-indigo-400">
          {enroll.finalAttendancePercentage != null ? `${enroll.finalAttendancePercentage.toFixed(1)}%` : "—"}
        </p>
      </div>
      <div className="w-px h-6 md:h-8 bg-slate-200 dark:bg-slate-700" />
      <div className="flex-1 text-center">
        <p className="text-[8px] md:text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Aprobado</p>
        <div className="flex items-center justify-center">
          {enroll.passed == null
            ? <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 text-slate-300 dark:text-slate-600" />
            : enroll.passed
    ...
    <div className="flex gap-2.5 md:gap-3 pt-1 md:pt-2">
      {/* ── FIX 1: disabled + spinner while fetching detail ────────────────── */}
      <button
        onClick={onView}
        disabled={detailLoading}
        className={`flex-1 py-3 md:py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-[1rem] md:rounded-2xl font-bold text-[10px] md:text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${detailLoading ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        {detailLoading
      ...
      {enroll.status === "ACTIVE" && (
        <button
          onClick={onWithdraw}
          disabled={detailLoading}
          className={`p-3 md:p-3.5 bg-rose-50 dark:bg-rose-900/30 text-rose-500 border border-rose-100 dark:border-rose-800/50 rounded-[1rem] md:rounded-2xl hover:bg-rose-100 transition-all shrink-0 ${detailLoading ? "opacity-50 cursor-not-allowed" : ""}`}
        >
