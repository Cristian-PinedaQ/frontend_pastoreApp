...
const ModalActivityParticipants = ({
  ...
  const loadParticipants = useCallback(async () => {
    ...
    try {
      ...
      const uniqueLeaders = [
        ...new Set(
          transformedParticipants.map((add) => add.leaderName).filter(Boolean),
        ),
      ];
      const uniqueDistricts = [
        ...new Set(
          transformedParticipants
            .map((add) => add.districtDescription)
            .filter(Boolean),
        ),
      ];
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiService.deleteActivityContribution(contributionId);
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      await confirm({
        ...
        message:
          "Ocurrió un error inesperado al intentar generar el PDF de participantes. Por favor, reintente en unos momentos.",
        type: "error",
  ...
  return (
    <>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 relative z-10 mb-6 sm:mb-8">
              <div className="flex items-center gap-4 sm:gap-6">
                ...
                <div>
                  ...
                  <h2 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tighter leading-tight uppercase truncate max-w-[250px] sm:max-w-none">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-6 sm:p-10 bg-slate-50 dark:bg-slate-950">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-8 sm:mb-10">
              {[
                ...
                {
                  label: "Deuda Total",
                  value: `$${stats.totalPending.toLocaleString("es-CO")}`,
                  icon: TrendingUp,
                  color: "indigo",
                },
              ].map((st) => (
                <div
                  key={st.label}
                  className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-white dark:border-slate-800 shadow-sm flex flex-col group"
                >
                  <div
                    className={`w-8 h-8 sm:w-10 sm:h-10 bg-${st.color}-500/10 text-${st.color}-500 rounded-lg sm:rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}
                  >
                    <st.icon size={16} className="sm:w-5 sm:h-5" />
                  </div>
                  <p className="text-sm sm:text-2xl font-black text-slate-900 dark:text-white tracking-tighter truncate">
                    {st.value}
                  </p>
            ...
            <div className="space-y-4">
              <h3 className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex justify-between items-center">
                ...
                <span className="text-indigo-600">
                  {filteredParticipants.length} Miembros
                </span>
              </h3>
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4 pb-4 border-b border-slate-50 dark:border-slate-800">
                          ...
                          <div className="text-right">
                            ...
                            <p
                              className={`text-xs font-black ${participant.pendingBalance > 0 ? "text-amber-500" : "text-emerald-500"}`}
                            >
                              $
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                          {filteredParticipants.map((participant) => (
                            ...
                            >
                              ...
                              <td className="px-8 py-5">
                                <p className="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-tight">
                                  {participant.leaderName || "SIN LÍDER"}
                                </p>
                              </td>
                              <td className="px-8 py-5">
                                <p className="text-[9px] font-black text-slate-400 uppercase">
                                  {participant.districtDescription || "N/A"}
                                </p>
                              </td>
                              <td className="px-8 py-5">
                                {participant.isFullyPaid ? (
                              ...
                              <td className="px-8 py-5">
                                <div className="items-center gap-2">
                                  ...
                                  <p
                                    className={`text-xs font-black ${participant.isFullyPaid ? "text-emerald-500" : "text-indigo-600"}`}
                                  >
                                    $
                                    {(
      ...
      {participantToDelete && (
        <div
          className="fixed inset-0 z-[1200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300"
        ...
        >
          ...
          >
            ...
            <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-4 leading-tight">
            ...
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-8 leading-relaxed">
              ...
              <span className="text-slate-900 dark:text-white font-black">
                {participantToDelete.memberName}
              </span>{" "}
              de esta actividad?
            </p>
            ...
            {(participantToDelete.totalPaid || 0) > 0 && (
              <div className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-2xl text-left mb-8 shadow-inner">
                <p className="text-[10px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest flex gap-3">
                  ...
                  <span>
                    Al eliminar a este miembro, desaparecerán sus aportes
                    registrados de{" "}
                    <span className="text-blue-600">
                      ${participantToDelete.totalPaid.toLocaleString()}
                    </span>{" "}
                    COP.
                  </span>
                </p>
