  ...
  useEffect(() => {
    const handler = (e) => {
      if (panelRef.current && !panelRef.current.contains(e.target) &&
          bellRef.current  && !bellRef.current.contains(e.target)) setOpen(false);
    };
    ...
    ? createPortal(
        ...
        >
          ...
          <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
            <div>
              <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tighter">Notificaciones</h3>
              {unreadCount > 0 && (
                <p className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase mt-0.5">{unreadCount} nuevas hoy</p>
              )}
            </div>
            {unreadCount > 0 && (
              <button
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
            ...
            {error && !loading && (
              <div className="p-6 text-center space-y-4">
                ...
                <button onClick={fetchNotifications} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 mx-auto">
            ...
            {!loading && !error && notifications.map((n) => {
              ...
              return (
                <div
                  ...
                  className={`
                    p-4 rounded-2xl transition-all duration-200 cursor-pointer flex gap-4 group mb-1
                    ${isUnread ? 'bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800' : 'opacity-60 grayscale-[0.5] hover:opacity-100 hover:grayscale-0'}
                  `}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[9px] font-bold text-slate-400">{timeAgo(n.createdAt)}</span>
                    </div>
                    <p className={`text-sm font-bold leading-snug tracking-tight ${isUnread ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                      {n.subject}
                    </p>
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-800 text-center text-slate-400">
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`
          w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center transition-all duration-300 relative
          ${open ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}
        `}
      >
        <Bell size={22} strokeWidth={open ? 2.5 : 2} />
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 min-w-[1.5rem] h-5.5 px-1.5 bg-rose-500 text-white text-[10px] font-black rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center shadow-md shadow-rose-500/40 z-10 transition-transform group-hover:scale-110">
