...
const WorshipRepertoireTab = ({
  ...
  const handleVocalistToggle = (memberId) => {
    setSongFormData((prev) => ({
      ...prev,
      vocalistIds: prev.vocalistIds.includes(memberId)
        ? prev.vocalistIds.filter((id) => id !== memberId)
        : [...prev.vocalistIds, memberId],
    }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center bg-white dark:bg-[#12141c] backdrop-blur-3xl p-5 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.05)] relative overflow-hidden group">
        ...
        <div className="relative md:flex-[4] w-full group/input">
          <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-indigo-500 transition-colors"></div>
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSongs.map((song) => (
          ...
          >
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex-grow">
                  <div className="flex flex-wrap gap-2 mb-3">
                    <span
                      className={`px-3 py-1 text-[9px] font-black uppercase tracking-[0.15em] rounded-xl border flex items-center gap-1.5 shadow-sm ${
                        song.type === "ALABANZA"
                    ...
                    >
                      {song.type === "ALABANZA" ? (
                        ...
                        <Heart className="w-3 h-3 stroke-[2.5px]" />
                      )}
                  ...
                  <div className="flex items-center gap-3 p-4 bg-indigo-500/5 dark:bg-indigo-600/10 rounded-[1.5rem] border border-indigo-500/10 ring-1 ring-white/5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-sm font-black text-indigo-900 dark:text-indigo-200 truncate italic">
                        {song.vocalists
                          .map((v) =>
                            getDisplayName(v.leader?.member?.name || v.name),
                          )
                          .join(", ")}
                      </p>
                    </div>
            ...
            {canManageWorship && (
              <div className="z-20 absolute top-4 right-2 flex flex-row gap-4 opacity-100 md:opacity-0 md:group-hover:opacity-100 translate-y-0 md:translate-y-2 md:group-hover:translate-y-0 transition-all duration-300">
                <button
                  onClick={() => openSongModal(song)}
                  className="flex items-center justify-center bg-white dark:bg-slate-800 text-slate-400 hover:text-indigo-600 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl transition-all active:scale-90 p-2"
                >
                ...
                >
                  <Trash2 className="w-8 h-8 text-violet-900" />
                </button>
              </div>
            )}
          </div>
        ...
        {filteredSongs.length === 0 && (
          <div className="col-span-full py-20 bg-slate-50/50 dark:bg-slate-900/20 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center text-center px-6">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 underline underline-offset-8 decoration-indigo-500/30">
              Sin resultados
            </h3>
            <p className="text-slate-500 dark:text-slate-400 max-w-xs font-medium">
              No se encontraron canciones que coincidan con tu búsqueda en el
              repertorio actual.
            </p>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[9999] overflow-y-auto overflow-x-hidden flex items-start sm:items-center justify-center p-2 sm:p-4 animate-in fade-in duration-300">
            ...
            <div className="relative w-full max-w-2xl my-auto bg-white dark:bg-[#12141c] rounded-[3rem] shadow-[0_0_100px_rgba(99,102,241,0.15)] overflow-hidden border border-slate-200 dark:border-white/10 animate-in zoom-in-95 fade-in slide-in-from-bottom-8 duration-500 max-h-[92vh] flex flex-col">
              ...
              <div className="px-8 pt-8 pb-4 flex justify-between items-center bg-gradient-to-b from-white/5 to-transparent shrink-0">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] mt-0.5">
                ...
                <button
                  onClick={() => !loading && setShowSongModal(false)}
                  className="flex items-center justify-center rounded-2xl bg-slate-100 dark:bg-white/5 hover:bg-rose-500 hover:text-white text-slate-500 transition-all active:scale-90"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar p-8 pt-4 space-y-8 scroll-smooth">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <ChevronRight className="absolute right-5 top-1/2  w-5 h-5 text-slate-400 rotate-90 pointer-events-none" />
                        </div>
                      </div>

                      <div className="space-y-5">
                        <label className="flex gap-2 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">
                          <Clock className="w-4 h-4  text-slate-400" />Tempo (BPM)
                        </label>
                        <div className="relative">
                          <input
                            type="number"
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest bg-indigo-500/10 px-3 py-1 rounded-lg">
                        03. Líderes de Alabanza Sugeridos
                      </span>
                      <div className="h-px flex-grow bg-slate-100 dark:bg-white/10" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-6 bg-slate-50 dark:bg-white/5 rounded-[2rem] border border-slate-200 dark:border-white/5 max-h-56 overflow-y-auto no-scrollbar shadow-inner">
                      {teamMembers
                        ...
                        .map((member) => {
                          const isSelected = songFormData.vocalistIds.includes(
                            member.id,
                          );
                          return (
                            <button
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${isSelected ? "bg-white/20" : "bg-slate-100 dark:bg-slate-700"}`}
                              >
                                {getDisplayName(member.memberName).charAt(0)}
                              </div>
                              <span className="text-[10px] font-black truncate">
                                {
                                  getDisplayName(member.memberName).split(
                                    " ",
                                  )[0]
                                }
                              </span>
