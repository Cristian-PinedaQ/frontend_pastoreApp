...
import {
  Calendar, Users, DollarSign, Search, Plus, Filter,
  ChevronDown, Clock, CheckCircle2, AlertCircle, History,
  LayoutGrid, List, RefreshCw, Wallet, TrendingUp, Zap,
  Target, FilterX, Loader2, ArrowRight, Download,
  CalendarCheck2, Layers, Edit2
} from 'lucide-react';
...
const STATUS_CONFIG = {
  ACTIVE:      { label: "Activa",      color: "emerald", icon: <CheckCircle2 className="w-4 h-4" /> },
  ENDING_SOON: { label: "Por Cerrar",  color: "amber",   icon: <Clock className="w-4 h-4" /> },
  FINISHED:    { label: "Finalizada",  color: "slate",   icon: <History className="w-4 h-4" /> },
...
const ActivityPage = () => {
  ...
  const [modals, setModals] = useState({
    add: false, details: false, participants: false, finance: false, edit: false
  });
  ...
  const filteredActivities = useMemo(() => {
    return allActivities.filter(a => {
      const matchesSearch = !searchTerm || a.activityName?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const stats = useMemo(() => {
    ...
    const totalRev = allActivities.reduce((acc, curr) => acc + (curr.price || 0), 0);
    return { active, total: allActivities.length, revenue: totalRev, finished: allActivities.length - active };
  }, [allActivities]);
  ...
  const handleSaveActivity = async (payload) => {
    try {
      ...
      setModals({ add: false, details: false, participants: false, finance: false, edit: false });
  ...
  const handleEnrollParticipant = async (activityId, memberId, initialPayment, incomeMethod, quantity) => {
    try {
      const payload = {
        activityId,
        memberId,
  ...
  const handleOpenFinance = async (activity) => {
    try {
      ...
      const balanceData = await apiService.request(`/activity/balance/${activity.id}`);
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-3 sm:p-6 md:p-10 space-y-6 md:space-y-10 animate-in fade-in duration-700">
        ...
        <div className="relative z-10 flex flex-col gap-6 sm:gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-[0.4em]">Agenda de Conquista</span>
            </div>
            <h1 className="font-black tracking-[-0.05em] text-slate-950 dark:text-white uppercase leading-[0.9]"
              style={{ fontSize: 'clamp(2rem, 8vw, 6rem)' }}>
              Agenda <br />
              <span className="bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent italic">Ministerial</span>
            </h1>
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <div className="px-3 py-2 sm:px-5 sm:py-2.5 bg-slate-100 dark:bg-white/5 rounded-xl flex items-center gap-2 border border-white/5">
                ...
                <span className="text-[9px] sm:text-[10px] font-black text-slate-500 dark:text-slate-300 uppercase tracking-widest">{stats.active} Actividades Vigentes</span>
              </div>
              <div className="px-3 py-2 sm:px-5 sm:py-2.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-xl flex items-center gap-2 border border-emerald-500/20">
                ...
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest">Planificación Estratégica</span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6">
        {[
          { label: 'Vigencia Actual',  value: stats.active,  icon: Zap,           color: 'emerald', sub: 'Eventos en curso' },
          { label: 'Alcance Global',   value: stats.total,   icon: Users,          color: 'indigo',  sub: 'Registros históricos' },
          { label: 'Inversión Total',  value: `$${new Intl.NumberFormat().format(stats.revenue)}`, icon: Wallet, color: 'violet', sub: 'Flujo proyectado' },
          { label: 'Ciclos Cerrados',  value: stats.finished, icon: CalendarCheck2, color: 'slate',   sub: 'Actividades finalizadas' },
        ].map((stat, i) => (
          <div key={i} className="group p-5 sm:p-7 bg-white dark:bg-[#1a2332] rounded-[2rem] sm:rounded-[3rem] border-2 border-slate-100 dark:border-white/5 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-500 relative overflow-hidden">
            <div className={`absolute -right-3 -top-3 w-32 h-32 bg-${stat.color}-500/5 rounded-full group-hover:scale-150 transition-transform duration-1000 blur-2xl`} />
            
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
              {/* Icon Section */}
              <div className={`w-14 h-14 sm:w-16 sm:h-16 bg-${stat.color}-500/10 text-${stat.color}-500 rounded-2xl flex items-center justify-center shrink-0 shadow-inner group-hover:scale-110 transition-transform`}>
                <stat.icon size={28} />
              </div>
      ...
      <div className="p-4 sm:p-6 md:p-10 bg-white dark:bg-[#0f172a] rounded-3xl sm:rounded-[3rem] md:rounded-[4rem] border border-slate-200 dark:border-white/10 shadow-sm space-y-5 md:space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-slate-100 dark:border-white/5">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">Fase de Actividad</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">Fecha Inicio</label>
            <input
              type="date"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">Fecha Cierre</label>
            <input
              type="date"
              value={dateRange.end}
          ...
          <div className="flex items-end">
            <button
              onClick={() => { setDateRange({ start: "", end: "" }); setSelectedStatus("ALL"); setSearchTerm(""); }}
      ...
      <div className="min-h-[300px]">
        {loading ? (
          <div className="py-20 sm:py-32 flex flex-col items-center gap-5">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse">Sincronizando Agenda Central...</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#0f172a] border-2 border-slate-100 dark:border-white/5 p-10 sm:p-20 rounded-3xl sm:rounded-[3rem] text-center shadow-sm">
            ...
            <h3 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase mb-3">Agenda Vacía</h3>
            <p className="text-slate-400 font-black uppercase tracking-[0.15em] text-[9px] sm:text-[10px] max-w-xs mx-auto">No se encontraron eventos coincidentes.</p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {filteredActivities.map((activity) => {
              ...
              return (
                <div key={activity.id} className={`group relative bg-white dark:bg-[#1a2332] rounded-3xl sm:rounded-[2.5rem] border-2 border-slate-100 dark:border-white/5 hover:border-indigo-500 transition-all duration-500 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 flex flex-col ${!activity.isActive ? 'opacity-60 grayscale' : ''}`}>
                  ...
                  <div className="p-5 sm:p-7 pb-3 flex-1 space-y-4 sm:space-y-5 relative overflow-hidden">
                    <div className={`absolute top-0 right-0 w-24 h-24 bg-${status.color}-500/5 -mr-10 -mt-10 rounded-full group-hover:scale-150 transition-transform duration-1000`} />

                    <div className="flex items-center justify-between relative z-10">
                      <div className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 bg-${status.color}-50 dark:bg-${status.color}-500/10 text-${status.color}-600 dark:text-${status.color}-400`}>
                        {status.icon} {status.label}
                      </div>
                      <span className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest">#{activity.id}</span>
                    </div>

                    <div className="space-y-2 relative z-10">
                      <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-tight group-hover:text-indigo-600 transition-colors">{activity.activityName}</h3>
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 dark:bg-black/40 text-indigo-600 dark:text-indigo-400 text-[8px] font-black uppercase tracking-widest rounded-full border border-indigo-100 dark:border-white/5">
                        <Layers size={9} /> {LEVEL_LABELS[activity.levelCode] || "General"}
                      </span>
                    </div>

                    <div className="grid grid-cols-2 gap-3 sm:gap-5 pt-4 mt-auto border-t border-slate-50 dark:border-white/5">
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><Wallet size={11} /> Inversión</p>
                        <p className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tighter">${new Intl.NumberFormat().format(Math.round(activity.price || 0))}</p>
                      </div>
                      <div className="space-y-1">
                  ...
                  <div className="p-3 sm:p-4 bg-slate-50 dark:bg-black/20 rounded-b-3xl sm:rounded-b-[2.5rem] border-t border-slate-100 dark:border-white/5">
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() => { setSelectedActivity(activity); setModals(prev => ({ ...prev, details: true })); }}
              ...
              <table className="w-full" style={{ minWidth: '600px' }}>
                <thead>
                  <tr className="bg-slate-50 dark:bg-black/40 border-b border-slate-100 dark:border-white/10">
                    <th className="px-5 sm:px-8 py-5 sm:py-7 text-left text-[10px] font-black uppercase text-slate-400 tracking-[0.25em]">Actividad</th>
                    <th className="px-5 sm:px-8 py-5 sm:py-7 text-left text-[10px] font-black uppercase text-slate-400 tracking-[0.25em] hidden sm:table-cell">Nivel</th>
                    <th className="px-5 sm:px-8 py-5 sm:py-7 text-left text-[10px] font-black uppercase text-slate-400 tracking-[0.25em]">Inversión</th>
                    <th className="px-5 sm:px-8 py-5 sm:py-7 text-left text-[10px] font-black uppercase text-slate-400 tracking-[0.25em]">Estado</th>
                    <th className="px-5 sm:px-8 py-5 sm:py-7 text-right text-[10px] font-black uppercase text-slate-400 tracking-[0.25em]">Acc.</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {filteredActivities.map((activity) => {
                    const status = getActivityStatus(activity);
                    return (
                      <tr key={activity.id} className="hover:bg-indigo-500/[0.02] transition-colors group">
                        ...
                        <td className="px-5 sm:px-8 py-5 sm:py-7 hidden sm:table-cell">
                          <span className="px-3 py-1.5 bg-slate-100 dark:bg-white/5 text-slate-500 text-[9px] font-black uppercase tracking-widest rounded-xl">{LEVEL_LABELS[activity.levelCode] || "General"}</span>
                        </td>
                        <td className="px-5 sm:px-8 py-5 sm:py-7">
                          <span className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tighter">${new Intl.NumberFormat().format(Math.round(activity.price || 0))}</span>
                        </td>
                        <td className="px-5 sm:px-8 py-5 sm:py-7">
                          <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest bg-${status.color}-50 dark:bg-${status.color}-500/10 text-${status.color}-600 dark:text-${status.color}-400`}>
                            {status.icon} <span className="hidden xs:inline">{status.label}</span>
                          </span>
                        </td>
                        <td className="px-5 sm:px-8 py-5 sm:py-7 text-right">
                          <div className="flex items-center justify-end gap-2">
                             <button onClick={() => handleEditActivity(activity)} className="p-2.5 sm:p-3 bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/20 rounded-xl border border-amber-100 dark:border-amber-500/20 transition-all shadow-sm" title="Editar"><Edit2 size={16} /></button>
                             <button onClick={() => handleOpenFinance(activity)} className="p-2.5 sm:p-3 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 rounded-xl border border-emerald-100 dark:border-emerald-500/20 transition-all shadow-sm" title="Finanzas"><DollarSign size={16} /></button>
                             <button onClick={() => { setSelectedActivity(activity); setModals(prev => ({ ...prev, details: true })); }} className="p-2.5 sm:p-3 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 rounded-xl border border-indigo-100 dark:border-indigo-500/20 transition-all shadow-sm" title="Ver Detalles"><ArrowRight size={16} /></button>
                             <button onClick={() => { setSelectedActivity(activity); setModals(prev => ({ ...prev, participants: true })); }} className="p-2.5 sm:p-3 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl border border-slate-200 dark:border-white/5 transition-all shadow-sm" title="Participantes"><Users size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <div className="relative p-6 sm:p-10 md:p-16 bg-indigo-600 rounded-3xl sm:rounded-[3rem] md:rounded-[5rem] text-white shadow-2xl shadow-indigo-600/20 overflow-hidden group mb-2">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-8 sm:gap-10">
          <div className="text-center sm:text-left space-y-3">
            <h3 className="font-black tracking-tighter uppercase" style={{ fontSize: 'clamp(1.5rem, 5vw, 3.5rem)', lineHeight: 1 }}>
              Consolidación <br /> Ministerial 2026
            </h3>
            <p className="text-white/40 font-black uppercase tracking-[0.4em] text-[9px] sm:text-[10px]">Raíz de David · Gestión Centralizada</p>
          </div>
          <div className="flex items-center gap-8 sm:gap-12">
            <div className="text-center">
              <p className="text-4xl sm:text-6xl font-black tracking-tighter text-white mb-1">{stats.total}</p>
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-white/50">Proyectos Totales</p>
            </div>
            <div className="w-px h-12 sm:h-16 bg-white/10" />
            <div className="text-center">
              <p className="text-4xl sm:text-6xl font-black tracking-tighter text-indigo-200 mb-1">{stats.active}</p>
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-white/50">Vigentes</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <ModalAddActivity 
        ...
        onClose={() => { setModals({ ...modals, add: false, edit: false }); setSelectedActivity(null); }} 
      ...
      <ModalActivityFinance 
        ...
        onClose={() => { setModals({ ...modals, finance: false }); setSelectedBalance(null); }} 
        activity={selectedActivity} 
