...
const FinancesPage = () => {
  ...
  const loadFinances = useCallback(async () => {
    ...
    try {
      ...
      const transformed = processedFinances.map((f) =>
        transformForDisplay(f, ["memberName"]),
      );
  ...
  const applyFilters = useCallback(() => {
    ...
    if (selectedConcept !== "ALL") {
      if (selectedConcept === "TITHE_AND_OFFERING") {
        filtered = filtered.filter(
          (f) => f.concept === "TITHE" || f.concept === "OFFERING",
        );
      } else {
    ...
    if (selectedVerification !== "ALL") {
      filtered = filtered.filter(
        (f) => f.isVerified === (selectedVerification === "VERIFIED"),
      );
    }
  ...
  const calculateDetailedStats = useCallback(() => {
    ...
    allFinances.forEach((f) => {
      ...
      if (f.isVerified) {
        s.verifiedAmount += f.amount;
        s.verifiedCount++;
      } else {
        s.unverifiedAmount += f.amount;
        s.unverifiedCount++;
      }
    ...
    try {
      if (type === "verify") {
        const isConfirmed = await confirm({
          ...
          message:
            "Esta acción validará formalmente el ingreso en el sistema contable.",
          type: "success",
        ...
        const isConfirmed = await confirm({
          ...
          message:
            "Esta operación es irreversible y afectará los reportes contables.",
          type: "danger",
        ...
        if (editingFinance)
          await apiService.updateFinance(editingFinance.id, payload);
    ...
    <div className="max-w-[1500px] mx-auto p-4 md:p-6 lg:p-10 space-y-6 md:space-y-10 animate-fade-in relative z-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
        <div className="bg-white dark:bg-[#1a2332] p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
          ...
          <p className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest mb-3 md:mb-4">
            Total Recaudado
          </p>
        ...
        <div className="bg-white dark:bg-[#1a2332] p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all flex flex-col justify-center gap-3 md:gap-4">
          ...
          >
            <span className="flex items-center gap-2.5">
              <RefreshCw
                className={`w-4 h-4 md:w-5 md:h-5 text-emerald-500 ${loading ? "animate-spin" : ""}`}
              />
              Sincronizar
            </span>
            <ChevronRight className="w-4 h-4 text-slate-400 group-hover:translate-x-1 transition-transform" />
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 p-4 md:p-6 rounded-[2rem] flex items-center justify-between group animate-shake">
          ...
          <button
            onClick={() => setError("")}
            className="text-red-400 hover:text-red-600 transition-colors bg-white/50 dark:bg-black/20 p-2 rounded-full"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-[#1a2332] border border-slate-200 dark:border-slate-800 rounded-[2rem] md:rounded-[3rem] shadow-sm overflow-hidden min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 md:py-32 text-center px-6 md:px-10">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-sm font-medium leading-relaxed text-sm">
              No encontramos registros que coincidan con los filtros
              seleccionados.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
                  <th className="px-6 md:px-8 py-5 md:py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                    Miembro Aportante
                  </th>
                  <th className="px-6 md:px-8 py-5 md:py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                    Monto
                  ...
                  <th className="px-6 md:px-8 py-5 md:py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 text-center">
                    Acciones
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {filteredFinances.map((f) => {
                  const leaderType = leaders.find(
                    (l) => l.memberId === f.memberId,
                  )?.leaderType;
                  const config =
                    CONCEPT_UI_CONFIG[f.concept] || CONCEPT_UI_CONFIG.DEFAULT;

                  return (
                    <tr
                      key={f.id}
                      className="group hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-all duration-300"
                    >
                      <td
                        className="px-6 md:px-8 py-4 md:py-6 cursor-pointer"
                        onClick={() => {
                          setEditingFinance(f);
                          setShowAddModal(true);
                        }}
                      >
                        <div className="flex items-center gap-4 md:gap-5">
                          ...
                          <div>
                            <p className="text-sm md:text-base lg:text-lg font-bold text-slate-900 dark:text-white leading-tight mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-1">
                              {f.memberName}
                            </p>
                      ...
                      <td className="px-6 md:px-8 py-4 md:py-6">
                        <div className="flex flex-col">
                          <p className="text-lg md:text-xl font-black text-slate-900 dark:text-white tracking-tighter whitespace-nowrap">
                            $ {f.amount.toLocaleString("es-CO")}
                          </p>
                      ...
                      <td className="px-6 md:px-8 py-4 md:py-6">
                        <span
                          className={`inline-flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 ${config.bgClass} ${config.textClass} text-[10px] md:text-[11px] font-black uppercase tracking-wider rounded-xl border ${config.borderClass} shadow-sm whitespace-nowrap`}
                        >
                          <div
      ...
      <div className="p-6 md:p-10 bg-indigo-600 dark:bg-indigo-900/50 border border-indigo-500/20 rounded-[2.5rem] md:rounded-[4rem] text-white flex flex-col xl:flex-row items-center justify-between gap-8 md:gap-10 shadow-xl relative overflow-hidden group">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 md:gap-8 relative z-10 w-full xl:w-auto">
          ...
          <div>
            <h4 className="text-2xl md:text-3xl font-black tracking-tight leading-none mb-3 md:mb-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4 text-indigo-100 text-xs font-bold uppercase tracking-widest">
              <span className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> Validación Concurrente
              </span>
              <span className="hidden sm:block w-1.5 h-1.5 bg-indigo-400 rounded-full"></span>
              <span className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4" /> Tiempo Real
              </span>
            </div>
          </div>
        </div>
        <div className="flex w-full xl:w-auto justify-center sm:justify-end gap-8 md:gap-12 relative z-10 bg-black/10 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-white/10">
          <div className="text-center">
            ...
            <p className="text-[9px] md:text-[10px] font-bold text-indigo-200 uppercase tracking-widest">
              Transacciones
            </p>
          </div>
          ...
          <div className="text-center">
            <p className="text-3xl md:text-4xl font-black mb-1">
              {((stats.verifiedCount / Math.max(stats.count, 1)) * 100).toFixed(
                0,
              )}
              <span className="text-lg md:text-xl text-indigo-300 ml-1">%</span>
            </p>
            <p className="text-[9px] md:text-[10px] font-bold text-indigo-200 uppercase tracking-widest">
