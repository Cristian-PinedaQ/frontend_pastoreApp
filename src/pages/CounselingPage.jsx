...
function ModalScheduleSession({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div
        className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[3rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden"
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                {isEditing
                  ? "Modificar detalles de la consejería"
                  : "Programar nueva cita pastoral"}
              </p>
            </div>
          </div>
          <button
              ...
              <div className="absolute z-10 w-full top-full mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-xl overflow-hidden max-h-48 overflow-y-auto backdrop-blur-xl">
                {filteredMembers.map((m) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold text-slate-400 group-hover:text-indigo-400 uppercase tracking-widest">
                      {m.document}
                    </span>
                  </button>
                ))}
  ...
  const handleSubmit = async () => {
    if (!form.notes.trim()) {
      await confirm({
        ...
        message:
          "Las notas y conclusiones son obligatorias para cerrar el expediente de la sesión.",
        type: "warning",
        confirmLabel: "Entendido",
      });
      return;
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">
              Conclusiones y Notas Pastorales *
            </label>
            <textarea
              rows={5}
          ...
          <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl space-y-4">
            <label className="flex items-center gap-4 cursor-pointer group">
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${form.followUpRequired ? "border-emerald-500 bg-emerald-500" : "border-slate-300 dark:border-slate-600"}`}
              >
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-500">
      ...
      >
        ...
        <div className="px-10 flex gap-4 border-b border-slate-200 dark:border-slate-800 overflow-x-auto no-scrollbar py-2">
          {[
            { id: "current", label: "Evolución Actual", icon: ClipboardList },
            {
              ...
              icon: History,
              badge: totalPreviousSessions,
            },
            {
              id: "followups",
              label: "Pendientes",
              icon: Bell,
              badge: pendingFollowUps.length,
              color: "rose",
            },
            ...
            >
              ...
              {tab.badge > 0 && (
                <span
                  className={`ml-1 px-2 py-0.5 rounded-lg text-[9px] ${tab.color === "rose" ? "bg-rose-500 text-white" : "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400"}`}
                >
                  {tab.badge}
                </span>
              )}
        ...
        {/* TAB CONTENT */}
        <div
          className="flex-1 overflow-y-auto p-10 bg-white dark:bg-slate-900"
          ref={scrollRef}
        >
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="bg-slate-50 dark:bg-slate-950 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                    <UserCheck className="w-4 h-4 text-indigo-500" /> Perfil del
                    Miembro
                  </h3>
                  <div className="space-y-4">
                    {[
                      {
                        label: "Documento",
                        value: activeSession.memberDocument || "N/A",
                        icon: FileText,
                      },
                      {
                        label: "Teléfono",
                        value: activeSession.memberPhone || "N/A",
                        icon: Phone,
                      },
                      {
                        label: "Correo",
                        value: activeSession.memberEmail || "N/A",
                        icon: Mail,
                      },
                    ].map((item) => (
                      <div
                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                    <Target className="w-4 h-4 text-indigo-500" /> Métrica de
                    Sesión
                  </h3>
                  <div className="space-y-4">
                    {[
                      {
                        ...
                        value:
                          COUNSELING_TOPICS[activeSession.topic] ||
                          activeSession.topic,
                        icon: Zap,
                      },
                      {
                        label: "Duración Estimada",
                        value: `${activeSession.durationMinutes} min`,
                        icon: Clock,
                      },
                      {
                        label: "Posición Histórica",
                        value: `Sesión #${activeSession.sessionNumber}`,
                        icon: BarChart3,
                      },
                    ].map((item) => (
                      <div
              ...
              {activeSession.objectives && (
                <div className="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                  ...
                  <p className="text-lg font-black leading-tight tracking-tight relative z-10 italic">
                    "{activeSession.objectives}"
                  </p>
                </div>
              )}
              ...
              <div className="space-y-4">
                ...
                />
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right mr-4 italic">
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              {!hasPrevious ? (
                <div className="py-20 text-center flex flex-col items-center">
                  ...
                  <h4 className="text-2xl font-black text-slate-900 dark:text-slate-200">
                    Paciente sin Antecedentes
                  </h4>
                  <p className="text-slate-400 font-medium mt-2">
                    Esta es la primera intervención registrada para este
                    miembro.
                  </p>
                </div>
              ) : (
                <div className="grid gap-6">
                  {previousSessions.map((s) => (
                    <div
                      ...
                      className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition-all group overflow-hidden relative"
                    >
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              {!hasFollowUps ? (
                <div className="py-20 text-center flex flex-col items-center">
                  ...
                  <p className="text-slate-400 font-medium mt-2">
                    Todo el seguimiento pastoral ha sido evacuado correctamente.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {pendingFollowUps.map((fu) => (
                    ...
                    >
                      ...
                      <p className="text-lg font-black text-slate-800 dark:text-slate-200 leading-tight mb-4">
                        {fu.followUpNotes}
                      </p>
              ...
              {!memberStats ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-4xl mx-auto">
                  {[
                    ...
                    {
                      label: "Cancelaciones",
                      value: memberStats.cancelledSessions,
                      icon: XCircle,
                      color: "rose",
                    },
                    {
                      label: "Incomparecencias",
                      value: memberStats.noShowSessions,
                      icon: Ghost,
                      color: "amber",
                    },
                    {
                      label: "Completadas",
                      value: memberStats.completedSessions,
                      icon: CheckCircle2,
                      color: "sky",
                    },
                    {
                      label: "Nivel Confianza",
                      value: memberStats.completionRate > 80 ? "ALTO" : "MEDIO",
                      icon: ShieldCheck,
                      color: "indigo",
                    },
                  ].map((stat) => (
                    <div
                      key={stat.label}
                      className="bg-slate-50 dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 text-center group hover:border-indigo-500/50 transition-all"
                    >
                      <div
                        className={`w-12 h-12 bg-${stat.color}-500/10 text-${stat.color}-500 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform`}
                      >
...
function ModalSessionDetail({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
      ...
      >
        {/* Detail Header */}
        {/* Detail Header - Notion Style */}
        <div className="relative p-8 pb-6 bg-white dark:bg-[#1f1f1f] border border-slate-200 dark:border-white/10 rounded-xl flex justify-between items-start">
          {/* CONTENT */}
          <div className="relative z-10">
            {/* META */}
            <div className="flex items-center gap-2 mb-3">
              <span className="px-2 py-0.5 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 text-[11px] rounded-md font-medium">
                ID {session.id} · Sesión #{session.sessionNumber}
              </span>

              <span className="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 text-[11px] rounded-md font-medium flex items-center gap-1">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-md hover:bg-slate-100 dark:hover:bg-white/5 transition-all active:scale-95"
          >
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-slate-50 dark:bg-slate-950/20">
          <div className="grid grid-cols-2 gap-8 p-8 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden">
            <div
              className={`absolute left-0 top-0 w-1.5 h-full bg-${status.color}-500`}
            ></div>
            <div>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                  <Zap className="w-3.5 h-3.5" /> TEMA:{" "}
                  <span className="text-slate-900 dark:text-white font-black">
          ...
          {session.notes && (
            <div className="bg-emerald-50 dark:bg-emerald-500/5 p-8 rounded-[2.5rem] border border-emerald-100 dark:border-emerald-500/20 shadow-inner">
              ...
              <p className="text-sm font-black text-slate-800 dark:text-slate-200 leading-relaxed uppercase tracking-tight">
                {session.notes}
              </p>
            </div>
          )}
          ...
          {session.cancellationReason && (
            <div className="bg-rose-50 dark:bg-rose-500/5 p-8 rounded-[2.5rem] border border-rose-100 dark:border-rose-500/20 shadow-inner">
              ...
              <p className="text-sm font-black text-rose-800 dark:text-rose-400 leading-relaxed uppercase tracking-tight">
                {session.cancellationReason}
              </p>
            </div>
          )}
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
      await confirm({
        ...
        message:
          "El motivo de cancelación es obligatorio para anular el registro.",
        type: "warning",
        confirmLabel: "Entendido",
      });
      return;
    ...
    setSaving(true);
    try {
      await onSave({ cancellationReason: reason });
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="p-8 space-y-6">
          <div className="p-6 bg-rose-50 dark:bg-rose-950/20 rounded-3xl border border-rose-100 dark:border-rose-900/40">
            ...
            <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
              Esta acción liberará el espacio agendado para{" "}
...
const CounselingPage = () => {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      if (list.length === 0) {
        console.warn(
          "⚠️ Gabinete Pastoral: No se encontraron sesiones registradas.",
        );
      }
    } catch (err) {
  ...
  const handleStartSession = useCallback(
    async (session) => {
      ...
      if (session.status === "IN_PROGRESS") {
        ...
        setShowDetail(false);
        try {
          const historyRaw = await apiService.request(
            `/counseling/member/${session.memberId}/history`,
          );
  ...
  const handleManagementPdf = async () => {
    try {
      ...
      await confirm({
        ...
        message:
          "Ocurrió un error al intentar generar el Reporte Maestro de Gestión.",
        type: "error",
        confirmLabel: "Entendido",
      });
    }
  ...
  const handleMemberHistoryPdf = async (session) => {
    try {
      await apiService.downloadCounselingMemberHistoryPdf(
        session.memberId,
        session.memberName,
      );
    } catch {
      await confirm({
        title: "Falla de Exportación",
        message:
          "Error al intentar generar la Ficha Pastoral del miembro. Verifique su conexión.",
        type: "error",
        confirmLabel: "Entendido",
      });
    }
  ...
  const handleNoShow = useCallback(
    async (session) => {
      ...
      await confirm({
        ...
        message:
          "¿Desea marcar este miembro como AUSENTE? Esta acción quedará registrada permanentemente en su historial pastoral.",
        type: "warning",
      ...
      try {
        if (type === "save") {
          ...
          await confirm({
            title: "¡Éxito!",
            message:
              "La operación se ha completado correctamente en el Gabinete Pastoral.",
            type: "success",
          ...
          await confirm({
            ...
            message:
              "La sesión ha sido cancelada y el espacio en agenda liberado.",
            type: "info",
            confirmLabel: "Entendido",
          });
          setShowCancel(false);
        ...
        await confirm({
          ...
          message:
            err.message || "Ocurrió un problema al procesar su solicitud.",
          type: "error",
          confirmLabel: "Cerrar",
        });
      } finally {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            icon: Activity,
            color: "emerald",
            sub: "Sesiones pulsantes",
            pulse: stats.inProgress > 0,
          },
          {
            label: "Efectividad Pastoral",
            value: `${stats.rate}%`,
            icon: BarChart3,
            color: "sky",
            sub: "Tasa de conclusión",
          },
        ].map((stat) => (
          <div
            key={stat.label}
            className="bg-white dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 rounded-[3rem] shadow-sm hover:shadow-xl transition-all group overflow-hidden relative"
          >
            <div
              className={`absolute top-0 right-0 w-24 h-24 bg-${stat.color}-500/5 dark:bg-${stat.color}-500/10 rounded-bl-[5rem] -mr-8 -mt-8 group-hover:scale-110 transition-transform`}
            ></div>
        ...
        <div className="flex items-center gap-4 ml-2">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
              Refine la búsqueda del expediente pastoral
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-900/40 backdrop-blur-xl p-20 rounded-[3.5rem] border border-slate-200 dark:border-slate-800 shadow-sm text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium max-w-sm mx-auto leading-relaxed italic">
              "En la paz de la justicia no se hallaron registros bajo estos
              criterios."
            </p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-slate-900/60 backdrop-blur-2xl p-10 rounded-[3rem] border shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden cursor-pointer active:scale-95 ${
                    isOngoing
                      ? "ring-4 ring-emerald-500/20 border-emerald-500/30"
                      : "border-slate-200 dark:border-slate-800"
                  }`}
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 bg-${status.color}-500/5 -mr-16 -mt-16 rounded-full group-hover:scale-150 transition-transform duration-700`}
                  ></div>

                  <div className="flex justify-between items-start mb-8 relative z-10">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-14 h-14 bg-${status.color}-500 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-${status.color}-500/20 group-hover:rotate-6 transition-transform duration-500`}
                      >
                    ...
                    {isOngoing ? (
                      <div className="p-2 bg-emerald-500 text-white rounded-full animate-pulse shadow-lg shadow-emerald-500/40">
                        <Zap className="w-4 h-4" />
                      </div>
                    ) : (
                  ...
                  <div className="space-y-4 mb-8 relative z-10">
                    <div className="flex items-center gap-3 text-xs font-bold text-slate-500 dark:text-slate-400">
                      <Calendar
                        className={`w-4 h-4 text-${status.color}-500`}
                      />
                      {formatDateTime(session.scheduledAt)}
                    </div>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-50 dark:border-slate-800 pt-6 relative z-10">
                    <div
                      className={`px-4 py-2 bg-${status.color}-50 dark:bg-${status.color}-500/10 text-${status.color}-600 dark:text-${status.color}-400 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl border border-${status.color}-100 dark:border-${status.color}-500/20`}
                    >
                      {status.label}
                    </div>
      ...
      <div className="bg-indigo-600 p-12 rounded-[4rem] text-white flex flex-col lg:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-6 items-center">
            {[
              ...
              <div
                key={item.label}
                className="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-xl rounded-2xl border border-white/10"
              >
        ...
        <div className="flex flex-col items-center lg:items-end gap-3 relative z-10">
          <p className="text-xs font-black uppercase tracking-[0.4em] opacity-40">
            Raíz de David · Gabinete Pastoral
          </p>
          <div className="h-1.5 w-32 bg-white/10 rounded-full overflow-hidden">
            <div
              className="h-full bg-white/60 animate-shimmer"
              style={{ width: "40%" }}
            ></div>
          </div>
        </div>
